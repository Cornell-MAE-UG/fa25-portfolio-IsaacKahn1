...
Each modified trellis post is extendable. Over-the-row netting is then installed over the trellis.
When the vines are ready for harvest, workers can extend the poles nearest to them, raising the netting. They can then harvest grapes, and lower the Adjustable Netting Posts behind them
